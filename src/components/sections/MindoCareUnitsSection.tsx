...
const MindoCareUnitsSection: React.FC = () => {
  ...
  const handleBookNow = (unitId: number) => {
    console.log("Book Now clicked for unit:", unitId);
  ...
  const renderCard = (unit: Unit, index: number) => {
    const isHovered = hoveredCard === unit.id;
    ...
    return (
      ...
      >
        {/* FRONT SIDE */}
        <div
          className={`relative z-10 bg-gradient-to-br from-mindo-blue/70 to-mindo-blue/50 rounded-3xl p-6 shadow-lg flex flex-col transition-opacity duration-300 ${
            isHovered
        ...
        {/* BACK SIDE */}
        <div
          className={`absolute inset-0 z-20 rounded-3xl overflow-hidden shadow-lg transition-opacity duration-300 ${
            isHovered
              ? "opacity-100 pointer-events-auto"
              : "opacity-0 pointer-events-none"
          }`}
        ...
        >
          <div className="relative w-full h-full flex flex-col">
            ...
            <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/40 to-transparent flex flex-col justify-end p-6">
              <h3 className="text-xl font-bold text-white mb-4">Unit Head</h3>
              <button
                type="button"
                onClick={() => handleBookNow(unit.id)}
                className="w-full py-2 px-4 rounded-full text-sm font-semibold text-gray-800 bg-gradient-to-r from-mindo-teal to-mindo-green hover:opacity-90 transition-all duration-200"
    ...
    >
      <div className="max-w-7xl mx-auto px-section-x">
        ...
        <div className="grid lg:grid-cols-12 gap-8 mb-16">
          <div className="lg:col-span-5">
            <p className="text-lg text-gray-700 mb-6">
              Our specialized care units bring together expert clinicians and
              personalized support.
            </p>
            <div className="rounded-3xl overflow-hidden shadow-lg">
              <img src={mindoUnit1} alt="Mindo Care Unit" />
            </div>
          </div>

          <div className="lg:col-span-7">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              {topUnits.map((unit, index) => renderCard(unit, index))}
            </div>
          </div>
        </div>
        ...
        <div className="grid lg:grid-cols-12 gap-8">
          <div className="lg:col-span-7">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              {bottomUnits.map((unit, index) => renderCard(unit, index + 6))}
            </div>
          </div>

          <div className="lg:col-span-5">
            <p className="text-lg text-gray-700 mb-6">
              Each unit is led by dedicated specialists who understand mental
              health deeply.
            </p>
            <div className="rounded-3xl overflow-hidden shadow-lg">
              <img src={mindoUnit2} alt="Mindo Care Unit" />
            </div>
          </div>
        </div>
      </div>
...
//   console.log("MindoCareUnitsSection rendered, navigate:", navigate);
